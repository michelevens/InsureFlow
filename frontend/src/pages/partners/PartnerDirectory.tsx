    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Find complementary service providers for your clients</p>
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        <button
          ...
          className={`text-sm px-3 py-1.5 rounded-full whitespace-nowrap transition-colors ${activeCategory === 'all' ? 'bg-shield-50 text-shield-700 font-medium' : 'text-slate-500 hover:bg-slate-100'}`}
        >All</button>
        {categories.map(cat => (
          <button
            key={cat}
            onClick={() => setActiveCategory(cat)}
            className={`text-sm px-3 py-1.5 rounded-full whitespace-nowrap transition-colors capitalize ${activeCategory === cat ? 'bg-shield-50 text-shield-700 font-medium' : 'text-slate-500 hover:bg-slate-100'}`}
          >{cat}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <Card className="p-12 text-center">
          ...
          <p className="text-slate-500 mb-4">No partners listed yet</p>
          <Button variant="shield" onClick={() => setShowCreate(true)}>
          ...
          {listings.map(listing => (
            <Card key={listing.id} className="p-5 hover:shadow-md transition-all">
              <div className="flex items-start gap-3 mb-3">
                {listing.logo_url ? (
                  ...
                  <div className="w-12 h-12 rounded-xl bg-shield-50 flex items-center justify-center">
                    <Handshake className="w-6 h-6 text-shield-600" />
                  </div>
                )}
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-slate-900">{listing.business_name}</h3>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-slate-400">({listing.review_count} reviews)</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-500 mb-3">
                {listing.website && (
                  <a href={listing.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-shield-600 hover:underline">
      ...
      {selectedPartner && (
        <Modal isOpen onClose={() => setSelectedPartner(null)} title={selectedPartner.business_name} size="md">
          <div className="space-y-4">
            ...
            {selectedPartner.description && <p className="text-sm text-slate-700">{selectedPartner.description}</p>}
            {selectedPartner.service_area && selectedPartner.service_area.length > 0 && (
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase mb-1">Service Area</p>
                <div className="flex flex-wrap gap-1">
                  {selectedPartner.service_area.map(a => <span key={a} className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{a}</span>)}
                </div>
              </div>
            )}
            <div className="space-y-2 text-sm text-slate-600">
              {selectedPartner.website && <div className="flex items-center gap-2"><Globe className="w-4 h-4" /> <a href={selectedPartner.website} target="_blank" rel="noopener noreferrer" className="text-shield-600 hover:underline">{selectedPartner.website}</a></div>}
    ...
    <Modal isOpen onClose={onClose} title="List Your Business" size="md">
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea value={description} onChange={e => setDescription(e.target.value)} className="w-full h-20 text-sm border border-slate-200 rounded-lg p-3 focus:ring-2 focus:ring-shield-500 focus:border-shield-500" placeholder="Describe your services..." />
        </div>
