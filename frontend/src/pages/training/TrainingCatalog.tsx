    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Build your skills with courses and certifications</p>
        </div>
      </div>
          ...
          <Card className="p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-shield-50 dark:bg-shield-900/30 flex items-center justify-center">
                <BookOpen className="w-5 h-5 text-shield-600 dark:text-shield-400" />
              </div>
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">Completed</p>
                <p className="text-xl font-bold text-slate-900 dark:text-white">{progress.completed_modules}/{progress.total_modules}</p>
              </div>
            </div>
          </Card>
          <Card className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-slate-900 dark:text-white">{progress.avg_score ? `${progress.avg_score}%` : 'N/A'}</p>
              </div>
            </div>
          </Card>
          <Card className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">Required Left</p>
                <p className="text-xl font-bold text-slate-900 dark:text-white">{progress.required_remaining}</p>
              </div>
            </div>
          </Card>
          <Card className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-slate-900 dark:text-white">{Math.round(progress.total_time_spent_minutes / 60)}h</p>
              </div>
            </div>
          </Card>
        </div>
      ...
      {progress && progress.total_modules > 0 && (
        <Card className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-slate-500 dark:text-slate-400">{Math.round((progress.completed_modules / progress.total_modules) * 100)}%</span>
          </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        <button
          ...
          className={`text-sm px-3 py-1.5 rounded-full whitespace-nowrap transition-colors ${
            category === 'all' ? 'bg-shield-50 dark:bg-shield-900/30 text-shield-700 dark:text-shield-300 font-medium' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800'
          }`}
        >
          All
        </button>
        {categories.map(cat => (
          <button
            ...
            className={`text-sm px-3 py-1.5 rounded-full whitespace-nowrap transition-colors ${
              category === cat ? 'bg-shield-50 dark:bg-shield-900/30 text-shield-700 dark:text-shield-300 font-medium' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800'
            }`}
          >
            {cat}
          </button>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {modules.map(mod => {
            ...
            return (
              <Card key={mod.id} className={`p-5 transition-all hover:shadow-md ${completed ? 'border-green-200 bg-green-50 dark:bg-green-900/30/30' : ''}`}>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-3">{mod.description}</p>
