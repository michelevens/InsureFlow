    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Track your earnings and commission payouts</p>
        </div>
      ...
      {isAgent && connectStatus?.stripe_onboarded && (
        <div className="bg-savings-50 dark:bg-savings-900/30 border border-savings-200 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CheckCircle className="w-5 h-5 text-savings-600 dark:text-savings-400" />
            <div>
              <p className="text-sm font-medium text-savings-900">Stripe Connected</p>
              <p className="text-xs text-savings-700 dark:text-savings-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 rounded-2xl bg-shield-100 dark:bg-shield-900/30 text-shield-600 dark:text-shield-400 flex items-center justify-center">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-savings-600 dark:text-savings-400">
                ${parseFloat(totalPaid).toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </p>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-savings-100 dark:bg-savings-900/30 text-savings-600 dark:text-savings-400 flex items-center justify-center">
      ...
      {activeTab === 'commissions' && (
        <Card>
          <div className="p-6">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Commission History</h2>
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-slate-600 dark:text-slate-300 font-medium">No commissions yet</p>
                <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Commissions will appear here when policies are bound</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-700/50">
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3">Policy</th>
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3 hidden sm:table-cell">Client</th>
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3 hidden md:table-cell">Carrier</th>
                      <th className="text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3">Premium</th>
                      <th className="text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3">Rate</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {commissions.map(c => {
                      ...
                      return (
                        <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800">
                          <td className="py-3 font-mono text-sm text-shield-600 dark:text-shield-400">
                            {c.policy?.policy_number || `#${c.policy_id}`}
                          </td>
                          <td className="py-3 text-sm font-medium text-slate-900 dark:text-white hidden sm:table-cell">
                            {c.policy?.user?.name || '—'}
                          </td>
                          <td className="py-3 text-sm text-slate-700 dark:text-slate-200 hidden md:table-cell">
                            {c.carrier_name}
                          </td>
                          <td className="py-3 text-right text-sm text-slate-700 dark:text-slate-200">
      ...
      {activeTab === 'payouts' && (
        <Card>
          <div className="p-6">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Payout History</h2>
            {payouts.length === 0 ? (
              <div className="py-12 text-center">
                <Banknote className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                <p className="text-slate-600 dark:text-slate-300 font-medium">No payouts yet</p>
                <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Request a payout when you have pending commissions</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-700/50">
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3">Date</th>
                      <th className="text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3">Amount</th>
                      <th className="text-right text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3 hidden sm:table-cell">Platform Fee</th>
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3 hidden md:table-cell">Period</th>
                      <th className="text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-3">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {payouts.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800">
                          ...
                          <td className="py-3 text-right text-sm text-slate-500 dark:text-slate-400 hidden sm:table-cell">
                          ...
                          <td className="py-3">
                            ...
                            {p.status === 'failed' && p.failure_reason && (
                              <div className="flex items-center gap-1 mt-1">
                                ...
                                <span className="text-xs text-red-600 dark:text-red-400">{p.failure_reason}</span>
                              </div>
                            )}
                          </td>
                        </tr>
