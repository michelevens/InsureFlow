  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Manage policy renewals and retain clients</p>
      </div>

      {/* Dashboard stats */}
      {dashboard && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Due in 30 days" value={dashboard.upcoming_30} icon={Calendar} color="bg-red-50 text-red-600" />
          <StatCard label="Due in 60 days" value={dashboard.upcoming_60} icon={Calendar} color="bg-amber-50 text-amber-600" />
          <StatCard label="At Risk" value={dashboard.at_risk} icon={AlertTriangle} color="bg-orange-50 text-orange-600" />
          <StatCard
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-slate-500 font-medium">No renewals found</p>
          <p className="text-sm text-slate-400 mt-1">Renewal opportunities appear as policies near expiration</p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl border p-5 transition-all ${
                  isUrgent ? 'border-red-200 bg-red-50/30' : 'border-slate-200 hover:border-shield-200'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      ...
                      <RefreshCw className={`w-5 h-5 ${isUrgent ? 'text-red-500' : 'text-slate-500'}`} />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right hidden sm:block">
                      <div className="flex items-center gap-1 text-sm text-slate-500">
                        ...
                        <span className={`ml-1 font-medium ${days <= 14 ? 'text-red-600' : days <= 30 ? 'text-amber-600' : 'text-slate-600'}`}>
                          ({days}d)
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-sm mt-0.5">
                        <DollarSign className="w-3.5 h-3.5 text-slate-400" />
                        <span className="text-slate-700 font-medium">${Number(renewal.current_premium).toLocaleString()}</span>
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <span className={`text-xs font-medium ${
                    renewal.retention_score >= 70 ? 'text-green-600' :
                    renewal.retention_score >= 40 ? 'text-amber-600' : 'text-red-600'
                  }`}>
