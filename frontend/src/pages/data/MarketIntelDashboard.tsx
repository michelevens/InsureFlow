  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Data & Analytics Products</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Premium market intelligence and performance insights</p>
        </div>
        <select
          value={period}
          onChange={e => setPeriod(e.target.value)}
          className="text-sm border border-slate-200 dark:border-slate-700/50 rounded-lg px-3 py-2"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-5">
              ...
              <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{marketData.total_quotes.toLocaleString()}</p>
            </Card>
            <Card className="p-5">
              <p className="text-sm text-slate-500 dark:text-slate-400">Avg Premium</p>
              <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">${marketData.avg_premium.toLocaleString()}</p>
            </Card>
            <Card className="p-5">
              <p className="text-sm text-slate-500 dark:text-slate-400">Top Carrier</p>
              <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{marketData.top_carriers[0]?.carrier || 'N/A'}</p>
            </Card>
          </div>

          {/* Top Carriers */}
          <Card className="p-5">
            <h3 className="font-bold text-slate-900 dark:text-white mb-4">Carrier Market Share</h3>
            <div className="space-y-3">
              {marketData.top_carriers.map((c, i) => (
                <div key={i} className="flex items-center gap-3">
                  <span className="text-sm text-slate-500 dark:text-slate-400 w-6">{i + 1}.</span>
                  <span className="text-sm font-medium text-slate-900 dark:text-white w-40 truncate">{c.carrier}</span>
                  ...
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-200 w-16 text-right">{c.market_share.toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          {/* Product Mix */}
          <Card className="p-5">
            <h3 className="font-bold text-slate-900 dark:text-white mb-4">Product Type Distribution</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {marketData.product_mix.map((p, i) => (
                <div key={i} className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-shield-600 dark:text-shield-400">{p.percentage.toFixed(1)}%</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{p.product_type}</p>
                </div>
              ))}
            </div>
          </Card>
          ...
          {/* Volume Trends */}
          <Card className="p-5">
            <h3 className="font-bold text-slate-900 dark:text-white mb-4">Volume Trends</h3>
              ...
              {marketData.trends.slice(-10).map((t, i) => (
                <div key={i} className="flex items-center justify-between text-sm py-1.5 border-b border-slate-50 last:border-0">
                  ...
                  <div className="flex items-center gap-6">
                    <span className="text-slate-900 dark:text-white font-medium">{t.volume} quotes</span>
                    <span className="text-slate-500 dark:text-slate-400">${t.avg_premium.toLocaleString()} avg</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          ...
          <Card className="p-6 text-center">
            ...
            <p className="text-4xl font-bold text-slate-900 dark:text-white">Top {100 - competitive.percentile_rank}%</p>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Your percentile rank in the market</p>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <Card key={i} className="p-5">
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">{m.label}</p>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-400 dark:text-slate-500">You</span>
                    <div className="flex items-center gap-1">
                      <span className="text-lg font-bold text-slate-900 dark:text-white">{m.yours}</span>
                      {m.better ? <ArrowUp className="w-4 h-4 text-green-500" /> : <ArrowDown className="w-4 h-4 text-red-500" />}
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-400 dark:text-slate-500">Market Avg</span>
                    <span className="text-sm text-slate-600 dark:text-slate-300">{m.market}</span>
                  </div>
                </div>
              </Card>
            ))}
          ...
          {competitive.improvement_areas.length > 0 && (
            <Card className="p-5">
              <h3 className="font-bold text-slate-900 dark:text-white mb-3">Areas for Improvement</h3>
              <div className="space-y-2">
                {competitive.improvement_areas.map((area, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm">
                    <div className="w-2 h-2 rounded-full bg-amber-400" />
                    <span className="text-slate-700 dark:text-slate-200">{area}</span>
                  </div>
                ))}
              </div>
            </Card>
        ...
        <Card className="p-5">
          <h3 className="font-bold text-slate-900 dark:text-white mb-4">Agent Leaderboard</h3>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-left text-xs text-slate-500 dark:text-slate-400 uppercase border-b border-slate-100 dark:border-slate-700/50">
                  ...
                  <tr key={b.agent_id} className="border-b border-slate-50 last:border-0">
                    <td className="py-3 pr-4">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                        b.rank <= 3 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                      }`}>
                        {b.rank}
                      </div>
                    </td>
                    <td className="py-3 pr-4 font-medium text-slate-900 dark:text-white">{b.agent_name}</td>
                    <td className="py-3 pr-4 text-right text-slate-700 dark:text-slate-200">{b.policies_written}</td>
                    <td className="py-3 pr-4 text-right text-slate-700 dark:text-slate-200">${b.total_premium.toLocaleString()}</td>
                    <td className="py-3 pr-4 text-right">
                      <Badge variant={b.conversion_rate > 0.3 ? 'success' : b.conversion_rate > 0.15 ? 'warning' : 'danger'}>
                        {(b.conversion_rate * 100).toFixed(1)}%
                      </Badge>
                    </td>
                    <td className="py-3 pr-4 text-right text-slate-700 dark:text-slate-200">{b.avg_response_time_hours.toFixed(1)}h</td>
                    <td className="py-3 text-right text-slate-700 dark:text-slate-200">{(b.retention_rate * 100).toFixed(0)}%</td>
                  </tr>
                ))}
                {benchmarks.length === 0 && (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-slate-400 dark:text-slate-500">No benchmark data available</td>
                  </tr>
                )}
              </tbody>
            </table>
