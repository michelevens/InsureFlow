    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Recruit, review, and hire insurance agents</p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-3">
            {postings.length === 0 ? (
              <Card className="p-12 text-center">
                ...
                <p className="text-slate-500 mb-4">No job postings yet</p>
                <Button variant="shield" onClick={() => setShowCreate(true)}>
              ...
              <Card key={posting.id} className={`p-4 cursor-pointer transition-all ${selectedPosting?.id === posting.id ? 'ring-2 ring-shield-500' : 'hover:shadow-md'}`} onClick={() => loadApplications(posting)}>
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-slate-900">{posting.title}</h3>
          ...
          <div>
            {selectedPosting ? (
              <Card className="p-5 sticky top-4">
                <h3 className="font-bold text-slate-900 mb-1">{selectedPosting.title}</h3>
                <p className="text-xs text-slate-500 mb-4">{applications.length} applications</p>
                <div className="space-y-3">
                  {applications.map(app => (
                    <div key={app.id} className="bg-slate-50 rounded-xl p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-slate-900 text-sm">{app.applicant_name}</span>
                        <Badge variant={statusColors[app.status]}>{app.status}</Badge>
                      </div>
                      <p className="text-xs text-slate-500">{app.applicant_email}</p>
                      {app.experience && (
                        <p className="text-xs text-slate-400 mt-1">{app.experience.years}y exp • {app.experience.specialties?.join(', ')}</p>
                      )}
                      <div className="flex gap-1 mt-2">
                        {['reviewing', 'interview', 'offered', 'hired', 'rejected'].filter(s => s !== app.status).slice(0, 3).map(s => (
                          <button
                            ...
                            className="text-xs px-2 py-1 rounded bg-white border border-slate-200 text-slate-600 hover:border-shield-300 capitalize"
                          >
                            {s}
                          </button>
                        ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card key={job.id} className="p-5">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <p className="text-sm text-slate-500">{job.agency_name}</p>
                </div>
          ...
          {publicJobs.length === 0 && (
            <Card className="p-12 text-center col-span-2">
              <Briefcase className="w-12 h-12 text-slate-300 mx-auto mb-3" />
              <p className="text-slate-500">No open positions available</p>
            </Card>
          )}
        </div>
      ) : null}
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full h-24 text-sm border border-slate-200 rounded-lg p-3 focus:ring-2 focus:ring-shield-500 focus:border-shield-500"
